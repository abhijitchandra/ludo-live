 ...
 * ui.js
 * -------------------------------------------------------
 * Handles the animated token layer, dice rendering, and
 * player panels. Tokens are absolutely positioned divs
 * that glide between cells using CSS transitions, stepping
 * through intermediate track cells one at a time so moves
 * always look "walked" rather than teleported.
 * -------------------------------------------------------
 */
(function () {
  ...
  let tokenLayer = null;
  let tokenEls = { red: [], yellow: [], green: [], blue: [] };
  let displayedSteps = { red: [-1, -1, -1, -1], yellow: [-1, -1, -1, -1] };
  let animating = false;

  function initTokenLayer(container) {
  ...
  async function animateToken(color, idx, fromSteps, toSteps) {
    ...
    if (fromSteps === -1 && toSteps >= 0) {
      // Leaving the yard: single hop onto the start square
      const [r, c] = E.stepsToCoord(color, 0, idx);
      placeTokenAt(el, r, c);
      await sleep(220);
    } else if (toSteps === -1) {
      // Captured: send home in one snappy hop
  ...
  /**
   * Diff old vs new tokens state and animate every changed token.
   * Captured tokens (other color reset to -1) animate too.
   */
  async function animateTokensTo(newTokens) {
    ...
    ['red', 'yellow'].forEach((color) => {
      newTokens[color].forEach((steps, idx) => {
        ...
        if (from !== steps) {
          jobs.push(animateToken(color, idx, from, steps));
        }
      });
    });
